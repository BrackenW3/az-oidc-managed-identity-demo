...
You can also customize the GitHub environment name (default: `Azure`) and the role assigned to the managed identity (default: `Reader`) by passing additional parameters:
...
4. Trigger the GitHub Actions workflow by navigating to **Actions** and choosing the "Run Azure Login" workflow. You can optionally specify the **GitHub Environment** name (default: `Azure`) to match your deployment. View the details of the 'demo' job in the workflow and inspect the steps.
...
1. The managed identity is authorized in Azure to a specific scope (Reader on the resource group in this case).
...
- In this demo, the user-assigned managed identity has a role-assigned to the resource group it is located in. By default, this is the Reader role, which means it cannot alter its own federated credentials. If you assign the Contributor role, it will have access to alter its own federated credentials.
